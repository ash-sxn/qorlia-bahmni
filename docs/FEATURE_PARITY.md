...
| Appointments | React admin service and unavailability screens exist. `/bahmni-v2/appointments/` is only a placeholder. | Build or integrate scheduling, calendars, booking, check-in, status changes and patient/provider views. The official home tile points to the separate legacy appointment frontend. |
